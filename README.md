...
## Extra extensions and dynamic linking

We added a patch which adds a number of extensions for automatic load and uses dynamic linking instead of static linking.
The man reason is the number of static libraries is very large and linking them all in the correct order is cumbersome.
Additionally the osx linker doesnt have `--start-group` and `--end-group` which makes it impossible to link all the
static libraries in the correct order on osx at all.

This fork also removs the windows and freebsd builds. The extensions are not available or not tested on those platforms.
...
* we use git-lfs to store the dynamic libraries. You must have git-lfs installed to download them.
* we use vcpkg to build dependencies. You must have vcpkg installed to build the dynamic libraries.

### Configure `git-lfs`
Simply `brew install git-lfs` to install git-lfs on osx. Then run `git lfs install` to configure git-lfs.

Finally `git lfs pull` to download the dynamic libraries. Note these steps are done automatically by golang when
building this project, however you musthave git-lfs installed.
